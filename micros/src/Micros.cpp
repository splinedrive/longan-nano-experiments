...
/* A port of Dmitry Sokolov's tiny raytracer to C and to FemtoRV33 */
...
/* adapted for longnano riscv by Hirosh Dabui, 2021 form https://github.com/splinedrive/kianRiscV */
...
//const int HRENDER = 160;
//const int VRENDER = 80;
...
static inline float maxr(float x, float y) { return x>y?x:y; }
static inline float minr(float x, float y) { return x<y?x:y; }
...
vec3 refract(vec3 I, vec3 N, float eta_t, float eta_i /* =1.f */) { // Snell's law
  float cosi = -maxr(-1.f, minr(1.f, vec3_dot(I,N)));
...
BOOL scene_intersect(vec3 orig, vec3 dir, Sphere* spheres, int nb_spheres, vec3* hit, vec3* N, Material* material) {
  ...
  return minr(spheres_dist, checkerboard_dist)<1000;
}
...
vec3 cast_ray(vec3 orig, vec3 dir, Sphere* spheres, int nb_spheres, Light* lights, int nb_lights, int depth /* =0 */) {
  ...
  for (int i=0; i<nb_lights; i++) {
    ...
    diffuse_light_intensity  += lights[i].intensity * maxr(0.f, vec3_dot(light_dir,N));

    float abc = maxr(0.f, vec3_dot(vec3_neg(reflect(vec3_neg(light_dir), N)),dir));
...
void set_pixel(volatile Pixel *fb, int x, int y, float r, float g, float b) {
  r = maxr(0.0f, minr(1.0f, r));
  g = maxr(0.0f, minr(1.0f, g));
  b = maxr(0.0f, minr(1.0f, b));
  ...
  G = minr((G>>4) + GOFF, 15);
  B = minr((B>>4) + BOFF, 15);
  LCD_DrawPoint_big(x, y, (R<<11) | (G<<6) | B);
  ...
  /*
     r = max(0.0f, minr(1.0f, r));
     g = max(0.0f, minr(1.0f, g));
     b = max(0.0f, minr(1.0f, b));
     switch(FGA_mode) {
     case GL_MODE_OLED: {
     uint8_t R = (uint8_t)(255.0f * r);
     uint8_t G = (uint8_t)(255.0f * g);
  ...
  printf("("rdcycle       :");
      ...
      //putchar(10);
        ...
        /*
           ...
           int minrutes = seconds / 60;
           int hours =   minrutes / 60;
        //print_str("seconds:");
